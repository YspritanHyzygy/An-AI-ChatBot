...
import { 
  ...
  BarChart3, Wrench, FolderOpen, Palette, History, Menu
} from 'lucide-react';
...
export default function Sidebar({
  ...
  const mainNavItems: NavItem[] = [
    ...
    {
      id: 'tools',
      label: '工具箱', // 占位符 - 需要实现工具集合
      icon: Wrench,
      children: [
        { id: 'tool1', label: '文本处理', icon: Wrench, path: '/tools/text' }, // 占位符路由
        { id: 'tool2', label: '图片生成', icon: Palette, path: '/tools/image' }, // 占位符路由
        // 在这里添加更多工具
      ]
    },
    {
      id: 'files',
      label: '文件管理', // 占位符 - 需要实现文件上传/管理功能
      icon: FolderOpen,
      path: '/files', // 路由未实现
      badge: 5 // 示例数字
    },
          ...
          onClick={() => {
            if (hasChildren) {
              ...
              if (item.path === '/') {
                ...
                // 占位符功能的提示
                alert(`功能「${item.label}」暂未实现，请等待后续版本更新。`);
              }
            } else {
              // 无路由的菜单项（如自定义功能）
        ...
        <div className="p-3 border-b border-gray-100 flex-shrink-0">
          ...
          >
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-gray-700">功能菜单</span>
            </div>
            <div className="flex items-center">
